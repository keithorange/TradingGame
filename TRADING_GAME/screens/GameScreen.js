...
const getRandomStockOhlc = () => {
  if (ohlcvDataSets.length > 0) {
    ...
    let i = 0;
    while (
      randomChoiceData.ohlcData.length < MIN_OHLC_DATA_LENGTH &&
      i < 1000
    ) {
...
const GameScreen = ({ route }) => {
  ...
  const [useBBCandles, setUseBBCandles] = useState(false);
  ...
  useEffect(() => {
    if (trailingStop) {
      ...
      if (pct < 0) {
        console.warn(
          "Negative trailing stop percentage detected, converting to positive.",
        );
      }
  ...
  async function fetchChartData() {
    ...
    const randomIndex =
      Math.floor(Math.random() * (maxPossibleIndex - minPossibleIndex + 1)) +
      minPossibleIndex;
  ...
  useEffect(() => {
    ...
    trades.forEach((trade, index) => {
      ...
      console.log(
        `Current Streak: ${currentStreak}, Current Streak Type: ${currentStreakType}`,
      );
      console.log(
        `Max Winning Streak: ${maxWinningStreak}, Max Losing Streak: ${maxLosingStreak}`,
      );
    });

    console.log(
      "Final Max Winning Streak:",
      maxWinningStreak,
      "Final Max Losing Streak:",
      maxLosingStreak,
    );
  ...
  const fastForwardChart = (skipAmount) => {
    ...
    const animationDurationMS = 200; // 2 seconds
    const skipTimePerCandle =
      animationDurationMS / (Math.log(skipAmount + 1) + 1);
          ...
          setTimeout(() => {
            ...
            if (newIndex < chartData.ohlcData.length) {
              ...
              if (trailingStopPct && position) {
                const slicedData = chartData.ohlcData.slice(
                  tradeStartIndex,
                  newIndex + 1,
                ); // Include current candle in calculation
                const { isHit, trailingStops, hitPrice } =
                  calculateTrailingStopSequence(
                    slicedData,
                    trailingStopPct,
                    position.direction,
                  );

                const currentTrailingStop =
                  trailingStops[trailingStops.length - 1];
  ...
  function executeTrade(direction) {
    if (!position) {
      ...
      const trade = handleTradeWithConditions(
        direction,
        chartData.ohlcData,
        tradeIndex,
        skipAmount,
  ...
  function displayResult(trade) {
    let resultMessage = trade.isWin
      ? `💵 Won! ${trade.roi.toFixed(2)}%`
      : `🔻 Loss ${trade.roi.toFixed(2)}%`;
  ...
  const calculateTrailingStopSequence = (
    data,
    trailingStopPct,
    direction,
    emaPeriod = 12,
    exitPriceIsStop = false,
  ) => {
    ...
    for (let i = 1; i < emaData.length; i++) {
      ...
      if (!hitDetected) {
        if (
          (direction === "Long" && ema <= stopPrice) ||
          (direction === "Short" && ema >= stopPrice)
        ) {
          hitDetected = true;
          //  if not tsl getting within-candle price,
          const kndl = data[i];
          const candleMean =
            (kndl["open"] + kndl["close"] + kndl["high"] + kndl["low"]) / 4;
          const candleMeanCloserToClose = (candleMean + kndl["close"]) / 2;
          ...
          console.log(
            ...
            "candleMean",
            candleMean,
            "candleMeanCloserToClose",
            candleMeanCloserToClose,
          );

          hitPrice = exitPriceIsStop ? stopPrice : candleMeanCloserToClose;
          hitIdx = i;
          console.log(
            `Trailing Stop Hit at index ${i}, Hit Price: ${hitPrice}`,
          ); // Debugging
        }
      }
  ...
  function handleTradeWithConditions(
    direction,
    data,
    index,
    skipInterval,
    use_mean_price = false,
    exitPriceIsStop = true,
  ) {
    console.log(
      "direction",
      direction,
    ...
    if (takeProfit || stopLoss || trailingStopPct) {
      for (let i = index + 1; i <= exitIndex; i++) {
        ...
        if (trailingStopPct) {
          const { isHit, hitPrice } = calculateTrailingStopSequence(
            data.slice(index, i + 1),
            trailingStopPct,
            direction,
            emaPeriod,
          );
  ...
  const MAXIMUM_SKIP_AMOUNT = 1000;
  ...
  const showingChartData = chartData.ohlcData.slice(
    currentChartIndex - candlesAmount,
    currentChartIndex + 1,
  ); // +1 to show CURRENT candle too!
    ...
    <View style={styles.container}>
      ...
      <View style={styles.headerContainer}>
        <StockInfoComponent
          ...
          allStockData={ohlcvDataSets.map(
            ({ ticker, humanName, category }) => ({
              ticker,
              humanName,
              category,
            }),
        ...
        <View style={styles.statsButtonContainer}>
          <View style={{ flexDirection: "row" }}>
            <Text
              style={[
                styles.totalRoiText,
                { color: totalROI >= 0 ? "rgba(0,148,32,1)" : "red" },
              ]}
            >
              {totalROI.toFixed(2)}%
            </Text>
            ...
            <Text
              style={[
                styles.totalRoiText,
                {
                  color: feeTotalROI >= 0 ? "rgba(0,148,32,1)" : "red",
                  paddingLeft: 10,
                  fontWeight: 300,
                },
              ]}
            >
              {feeTotalROI.toFixed(2)}%
            </Text>
          </View>
          <View>
            <Text
              style={[styles.streakText, losingStreak > 0 && { color: "red" }]}
            >
      ...
      <View style={styles.chartContainer}>
        ...
        <View style={{ position: "absolute", top: 20, left: 20 }}>
          <Text
            style={{
              color: LIGHT_MODE ? "#808e9b" : "#d2dae2",
              fontStyle: "underline",
              fontSize: 18,
              textDecorationLine: "underline",
            }}
          >
            Length: {candlesAmount}
          </Text>
          <Slider
        ...
        <View style={styles.controlMenu}>
          ...
          <View style={{ flexDirection: "row" }}>
            ...
            >
              <TouchableOpacity
                style={[
                  styles.longshortButton,
                  { backgroundColor: "#4cd137", borderRadius: 50 },
                ]}
              ...
              <TouchableOpacity
                style={[
                  styles.longshortButton,
                  { backgroundColor: "#EA2027", borderRadius: 50 },
                ]}
            ...
            >
              ...
              >
                <Text
                  style={[
                    styles.tpsltsButtonText,
                    { color: takeProfit ? "white" : "#2196F3" },
                  ]}
                >
              ...
              <TouchableOpacity
                style={[
                  styles.tpsltsButton,
                  {
                    borderColor: trailingStop
                      ? "rgba(255,153,221,1)"
                      : "rgba(255,153,221,1)",
                    backgroundColor: trailingStop
                      ? "rgba(255,153,221,1)"
                      : "white",
                  },
                ]}
              ...
              >
                <Text
                  style={[
                    styles.tpsltsButtonText,
                    { color: trailingStop ? "white" : "rgba(255,153,221,1)" },
                  ]}
                >
              ...
              >
                ...
                <View style={{
                  ...
                  <Switch
                    value={useBBCandles}
                    onValueChange={(value) => setUseBBCandles(value)}
                  />
                </View>
              <View
                style={{
